...
class XTouchMiniMixer {
 public:
  ...
  void setup(void (*p_func_usb)(void) = nullptr);
  void setupMidi();
  void update();
  ...
  void registerDebuggingCallbacks() {
    DEBUG_PRINTLN("registerDebuggingCallbacks");
 ...
 private:
  ...
  static void mixChannelPrintln(uint8_t id, uint8_t id_bus, uint8_t val);
