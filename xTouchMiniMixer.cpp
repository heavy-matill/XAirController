...
void XTouchMiniMixer::setup(void (*p_func_usb)(void)) {
  DEBUG_PRINTLN("initializing XTouch");
  // registerDebuggingCallbacks();
...
void XTouchMiniMixer::setMixed(uint8_t id, uint8_t id_bus, uint8_t val) {
  mixes[id_bus][id] = val;
...
void XTouchMiniMixer::onButtonUp(uint8_t id) {
  ...
  if (40 <= id && id < 48) {
    ...
    return muteChannelCallback(id - 32, getMuteValue(id - 32));
  }
      ...
      if (st_control == 0) {
        ...
        for (uint8_t id_bus = 0; id_bus < 6; id_bus++) {
          if (st_control & (1 << (2 + id_bus))) {
            mixChannelCallback(id, id_bus, val);
          }
        }
      }
  }
