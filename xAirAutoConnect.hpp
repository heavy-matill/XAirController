...
void onRoot() {
  ...
  webServer.sendHeader("Location", String("http://") +
                                       webServer.client().localIP().toString() +
                                       String(HELLO_URI));
...
String onHello(AutoConnectAux& aux, PageArgument& args) {
  ...
  if (dir) {
    ...
    while (parmFile) {
      if (!parmFile.isDirectory()) styles.add(String(parmFile.name()));
  ...
  while (dir.next()) {
    if (!dir.isDirectory()) styles.add(dir.fileName());
  }
#endif
...
void loopAuto() { portal.handleClient(); }